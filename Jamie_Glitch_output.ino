...
const char* myID = "Jamie"; // place your name here, this will be put into your "sender" value for an outgoing messsage

char publishChannel[] = "JAMIE"; // channel to publish YOUR data
char readChannel[] = "ACHAL"; // channel to read THEIR data
...
int ACHALPIN;  ///the value Nick reads from Kates board via PN
int JAMIEPIN; //the value I get locally

int tp = 4; //change this if you aren't using them all, start at pin 0
int ledPins[] = {2, 3, 4, 5};
int ledTotal = sizeof(ledPins) / sizeof(int);
boolean BlinkState[] = {false, false, false};
int achalLastPinTouched = 0;
...
{
   MyLeds();
  checkAllPins(tp); //read the capacitive sensor

  sendReceiveMessages(serverCheckRate);  //send and receive messages with PubNub, based on a timer
}
...
void sendMessage(char channel[])
{
  ...
  dataToSend[JsonParamName2] = JAMIEPIN; // second key value is the Pin number
...
void readMessage(char channel[])
{
  ...
  while (!getMessage.finished())
  {
    ...
    if (msg.length() > 0)
    {
      ...
      ACHALPIN = inMessage[JsonParamName2]; // the value of their Temperature sensor

      Serial.println(ledPins[ACHALPIN]);
      Serial.println(BlinkState[ACHALPIN]);
      Serial.println(ACHALPIN);

      if (ACHALPIN != achalLastPinTouched) {
        if (ACHALPIN == 0) {
          if (BlinkState[0]) {
            BlinkState[0] = false;
            digitalWrite(0, LOW);
 ...
 /**  if (BlinkState[ACHALPIN] == true) {
           digitalWrite(ledPins[JAMIEPIN], BlinkState[ACHALPIN]);
           //delay(1000);
           Serial.println(ledPins[ACHALPIN]);
           Serial.println(BlinkState[ACHALPIN]);
         }
